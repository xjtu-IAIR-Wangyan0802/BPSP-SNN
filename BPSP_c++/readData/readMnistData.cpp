...
int read_Mnist(std::string filename, 
	...
	int flag){
        ...
        std::default_random_engine dre(960802);
		int id = 0;
		if (file.is_open()){
...
//generatePoissonSpikes(x, data, num, input_neurons, end_time);
    ...
    )
{
    ...
    std::mt19937 e2(rd());
    std::uniform_real_distribution<> dist(0, 1);
    std::default_random_engine dre(9682);
    ...
    for(int i = 0; i < data.size(); ++i){
        ...
        for(int j = 0; j < data[i]->rows; ++j){
            for(int k = 0; k < data[i]->cols; ++k){
                float freq = data[i]->get(j, k, 0); //大部分是0
                // map the freq to (-1, 1) for future processing
                // 把图像中的值从[0,1]映射至(-1,1)，便于未来进行处理
                data[i]->set(j, k, 0, (freq * CONST_SPIKING_SCALE) * 2.0f / 255.0f - 1.0f);
                //在原始图像中增加椒盐噪声 dist()生成【0,1】之间的随机数
                // if (num == 60000){
                //     float ratio = 0.45;
                //     float random_number = dist(dre);
                //     if (random_number < ratio){
                //         if (random_number < 0.5 * ratio){
                //             freq = 1.0;}
                //         else{
                //             freq = 0.0;
                //         }
