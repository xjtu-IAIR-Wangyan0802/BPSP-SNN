...
void __global__ g_boostWeightUpdate(float* weights, bool* predictions, int* y, int len, int nclasses)
{
    ...
    // compute the sum of the boosting weight for each class
...
void predictTestRate(cuMatrixVector<bool>&x,
    ...
    DataLayerSpiking *dl = static_cast<DataLayerSpiking*>(Layers::instance()->get("data"));//static_cast强制类型转换
        ...
        cublasHandle_t handle)
{
    ...
    if(Config::instance()->getIsGradientChecking())
        gradientChecking(x.m_devPoint, y->getDev(), batch, nclasses, handle);//函数是空的，x,y-->trainX,trainY
    ...
    for (int epo = 0; epo < epochs; epo++) {
        ...
        x.shuffle(5000, y, cuSampleWeight);//x->TrainX，shuffle将序列的所有元素随机排序
