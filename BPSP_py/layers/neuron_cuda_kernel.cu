...
__global__ void neuron_forward_cuda_kernel(
    ...
    scalar_t *__restrict__ threshold,
    const float theta_m, const float theta_s, const float theta_grad,
    const float is_forward_leaky, const float is_grad_exp,
    size_t neuron_num, size_t tot_size, size_t n_channel, size_t feature_map_size) {
    
    size_t index = blockIdx.x * blockDim.x + threadIdx.x;
    float syn_m = 0, syn_s = 0, syn_grad = 0, u_last = 0, u = 0, out = 0;

    if (index < neuron_num) {
        size_t channel_idx = index / feature_map_size % n_channel;
...
__global__ void neuron_backward_cuda_kernel(
    ...
    int count_outputs =1 , count_inputs =1;
    float ratio =1.0;
    if (index <neuron_num){
        for (index = tot_size - neuron_num + index; index >= 0; index -= neuron_num){
            ...
            if  (delta_u_t[index]!=0){
                count_inputs +=1;
            }
        }
        ratio = (count_outputs + count_inputs)/count_outputs;
        for (index = tot_size - neuron_num + index; index >= 0; index -= neuron_num){
            out = outputs[index] > 0;
            spiked |= out;
            ...
            grad_in_[index] = spiked ? partial_u_grad_t * ratio *  (-0.1) * exp(- delta_t/2 ) : 0; 
            grad_w_[index] = spiked ? partial_u_grad_w * ratio * 0.1 * (-0.5 * exp( -1.0 *delta_t /2) + 1) : 0;
        }
    }
}
...
std::vector<torch::Tensor> neuron_forward_cuda(
    ...
    const auto n_channel = in_I.size(2), feature_map_size = neuron_num / in_I.size(1) / n_channel;
    ...
    AT_DISPATCH_FLOATING_TYPES(in_I.type(), "neuron_forward_cuda_kernel", ([&] {
                                   neuron_forward_cuda_kernel<scalar_t><<<blocks, threads>>>(
                                       ...
                                       threshold.data<scalar_t>(),
                                       theta_m, theta_s, theta_grad, is_forward_leaky, is_grad_exp,
                                       neuron_num, tot_size, n_channel, feature_map_size);
                               }));
