...
def init(config_n, config_l=None):
    ...
    if 'readout' not in network_config.keys():
        network_config['readout'] = 'None'
    ...
    if 'data_aug' not in network_config.keys():
        network_config['data_aug'] = 'strong'
    if 'norm_threshold' not in network_config.keys():
        network_config['norm_threshold'] = True
    if 'lr_norm' not in network_config.keys():
        network_config['lr_norm'] = network_config['lr']
    ...
    if 'weight_decay_norm' not in network_config:
        network_config['weight_decay_norm'] = network_config['weight_decay']
